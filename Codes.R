...
Starter_data <- Starter_data %>%
  ...
  mutate(across(.cols = c(IP,ER,luck,WHIP,FIP,BBK,IPHR),
                .fns = list(
                  ...
                  movavg = ~ slide_dbl(., mean, .before = 6, .complete = FALSE)  # 이동 평균 계산
                )))
